...
library(ggplot2)
library(cowplot)
plotPC(pca.data = df5, data = df4, group = "Time", plot_type = "DensityPlot", npcs = 3)
```
...
```{r}
df9 <- createPRPS(data=df4, batch=c('Year', 'Plates'), biology='biology', purity=NULL, include.ls=T, include.purity=F, n.ls.ps=10, n.sample.batch=3, n.sample.purity=0, n.sample.ls=3)
```
...
plotPC(pca.data = df11, data = combined_data, group = "Time", plot_type = "DensityPlot", npcs = 3)
```
