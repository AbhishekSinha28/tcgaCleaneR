...
raw.count <- as.data.frame(SummarizedExperiment::assay(brca.se.data.temp, 'HTseq_counts'))
```
# Data Wrangling
...
```{r}
df1 <- filter.genes(data=brca.data,gene.type=c("protein_coding"))
```
...
```{r}
df7 <- gene.corr(data = df4, is.log = FALSE, type = "librarysize", cor.method = 'spearman', n.cores = 5)
head(df7)
```
...
```{r}
df8 <- f.test(data = df4, variable = "Time", is.log = FALSE, n.cores = 5)
head(df8)
```
