...
```{r}
df4 <- library.size.filter(data = df3, ls_cutoff = 17.5)
```
...
```{r fig.align='center', fig.width=7, fig.height=5}
df7 <- gene.corr(data = df4, is.log = FALSE, type = "librarysize", cor.method = 'spearman', n.cores = 1)
head(df7)
```
...
```{r}
df8 <- anova_test(data = df4, variable = "Time", is.log = FALSE, n.cores = 1)
head(df8)
```
# RUV - III
...
```{r}
pca.plot(pca.data = df11, data = combined_data, group = "Time", plot_type = "DensityPlot", npcs = 3)
```
